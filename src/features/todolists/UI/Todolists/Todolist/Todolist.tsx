...
import { FilterValueType } from "../../../../../app/AppAndAppWithReducer/App";
...
export const Todolist = ({
  ...
  function filterTasks(allTask: DomainTask[], filter: FilterValueType) {
    switch (filter) {
      case "active":
        return allTask.filter((task) => task.status !== TaskStatus.Completed);
      case "completed":
        return allTask.filter((task) => task.status !== TaskStatus.New);
      default:
        return allTask;
    }
  }
  ...
  return (
    <div>
      <h3>
        <EditableSpan title={title} onChange={changeNewTitleTodolistHandler} />
      </h3>
      ...
      {tasksList}
      <div>
        <Button variant={filter === "all" ? "contained" : "outlined"} onClick={onAllClickHandler}>
          All
        </Button>
        <Button variant={filter === "active" ? "contained" : "outlined"} onClick={onActivelClickHandler}>
          Active
        </Button>
        <Button variant={filter === "completed" ? "contained" : "outlined"} onClick={onCompletedClickHandler}>
          Completed
        </Button>
      </div>
    </div>
